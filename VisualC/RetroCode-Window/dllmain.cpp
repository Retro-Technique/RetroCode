...
static AFX_EXTENSION_MODULE RetroCodeWindowDLL = { false, nullptr };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("Initialisation de RetroCode-Window.DLL !\n");

		// Initialisation unique de la DLL d'extension
		if (!AfxInitExtensionModule(RetroCodeWindowDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(RetroCodeWindowDLL);

	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("Fin d'exécution de RetroCode-Window.DLL !\n");

		// Terminez la bibliothèque avant que les destructeurs soient appelés
		AfxTermExtensionModule(RetroCodeWindowDLL);
	}
	return 1;   // ok
}
