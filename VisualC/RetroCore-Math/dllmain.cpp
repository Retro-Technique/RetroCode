...
static AFX_EXTENSION_MODULE RetroCoreMathDLL = { false, nullptr };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("Initialisation de RetroCore-Math.DLL !\n");

		// Initialisation unique de la DLL d'extension
		if (!AfxInitExtensionModule(RetroCoreMathDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(RetroCoreMathDLL);

	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("Fin d'exécution de RetroCore-Math.DLL !\n");

		// Terminez la bibliothèque avant que les destructeurs soient appelés
		AfxTermExtensionModule(RetroCoreMathDLL);
	}
	return 1;   // ok
}
