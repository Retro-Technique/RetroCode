...
static AFX_EXTENSION_MODULE RetroCodeGeometryDLL = { false, nullptr };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("Initialisation de RetroCode-Geometry.DLL !\n");

		// Initialisation unique de la DLL d'extension
		if (!AfxInitExtensionModule(RetroCodeGeometryDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(RetroCodeGeometryDLL);

	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("Fin d'exécution de RetroCode-Geometry.DLL !\n");

		// Terminez la bibliothèque avant que les destructeurs soient appelés
		AfxTermExtensionModule(RetroCodeGeometryDLL);
	}
	return 1;   // ok
}
