...
static AFX_EXTENSION_MODULE RetroCodeMultimediaDLL = { false, nullptr };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("Initialisation de RetroCode-Multimedia.DLL !\n");

		// Initialisation unique de la DLL d'extension
		if (!AfxInitExtensionModule(RetroCodeMultimediaDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(RetroCodeMultimediaDLL);

	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("Fin d'exécution de RetroCode-Multimedia.DLL !\n");

		// Terminez la bibliothèque avant que les destructeurs soient appelés
		AfxTermExtensionModule(RetroCodeMultimediaDLL);
	}
	return 1;   // ok
}
