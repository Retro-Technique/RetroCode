...
static AFX_EXTENSION_MODULE RetroCodeImageDLL = { false, nullptr };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("Initialisation de RetroCode-Image.DLL !\n");

		// Initialisation unique de la DLL d'extension
		if (!AfxInitExtensionModule(RetroCodeImageDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(RetroCodeImageDLL);

	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("Fin d'exécution de RetroCode-Image.DLL !\n");

		// Terminez la bibliothèque avant que les destructeurs soient appelés
		AfxTermExtensionModule(RetroCodeImageDLL);
	}
	return 1;   // ok
}
