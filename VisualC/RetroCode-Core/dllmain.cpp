...
static AFX_EXTENSION_MODULE RetroCodeCoreDLL = { false, nullptr };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("Initialisation de RetroCode-Core.DLL !\n");

		// Initialisation unique de la DLL d'extension
		if (!AfxInitExtensionModule(RetroCodeCoreDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(RetroCodeCoreDLL);

	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("Fin d'exécution de RetroCode-Core.DLL !\n");

		// Terminez la bibliothèque avant que les destructeurs soient appelés
		AfxTermExtensionModule(RetroCodeCoreDLL);
	}
	return 1;   // ok
}
